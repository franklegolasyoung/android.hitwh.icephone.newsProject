...
public class ThreeFragment extends Fragment {
    ...
    public List<News.DataBean.SportsBean> sNewsList = new ArrayList<>();
        ...
        new Thread(new Runnable() {
            ...
            public void run() {
                try {/*                    OkHttpClient client = new OkHttpClient();
                    ...
                    tNewsList = news.getData().getSports();*/
                    ...
                    try {
                        ...
                        Gson gson = new Gson();
                        News news = gson.fromJson(responseData, News.class);
                        sNewsList = news.getData().getSports();
                    } catch (IOException e) {
                        e.printStackTrace();
                    }
                } catch (Exception e) {
    ...
    private SportsNewsAdapter tAdapter;//自定义recycleview的适配器
    ...
    public View onCreateView(LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        ...
        //创建adapter
        tAdapter = new SportsNewsAdapter(sNewsList);
        //给RecyclerView设置adapter
        mrecyclerView.setAdapter(tAdapter);
        //设置item的分割线
        mrecyclerView.addItemDecoration(new DividerItemDecoration(getActivity(), DividerItemDecoration.VERTICAL));
        //RecyclerView中没有item的监听事件，需要自己在适配器中写一个监听事件的接口。参数根据自定义
        tAdapter.setOnItemClickListener(new SportsNewsAdapter.OnItemClickListener() {
            @Override
            public void onItemClick(View view, int position) {
                //此处进行监听事件的业务处理
                News.DataBean.SportsBean news = sNewsList.get(position);
